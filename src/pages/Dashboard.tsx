...
function EditableField({
  ...
  if (isEditing) {
    return <div className="space-y-3">
        ...
        {multiline ? <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="input-field min-h-[100px] resize-y" autoFocus /> : <input type="text" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="input-field" autoFocus />}
        <div className="flex gap-2">
  ...
  return <div className="group">
      {label && <label className="label-text">{label}</label>}
      <div onClick={onEdit} className="p-4 rounded-lg border border-transparent bg-muted/30 hover:bg-muted/50 hover:border-border cursor-pointer transition-all duration-200 min-h-[60px]">
